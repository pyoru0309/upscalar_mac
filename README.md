...
```bash
git clone https://github.com/pyoru0309/upscalar_mac.git
cd upscalar_mac
python3 -m venv .venv
...
- `src-tauri/target/release/bundle/macos/Upscalar.app` … Finderからダブルクリックで起動
- `src-tauri/target/release/bundle/dmg/Upscalar_<ver>_aarch64.dmg` … 配布用ディスクイメージ
...
- 起動URLは `app.py` が出力する `UPSCALAR_URL=...` マーカーから取得するため、動的ポートでも確実に追従します。
- ウィンドウを閉じる/アプリを終了すると Gradio サーバも停止します。さらに `UPSCALAR_PARENT_PID` を渡しており、ラッパーがクラッシュ／強制終了してもPython側が親プロセスの消滅を検知して自発終了します。
- Python実行ファイルは `.venv` を自動検出します。変更する場合は `UPSCALAR_PYTHON` で上書きできます。
...
```bash
python -m upscalar.cli --input inputs/example.png --engine pillow_lanczos --scale 2
python -m upscalar.cli --input inputs/ --recursive --engine pillow_lanczos --scale 2
python -m upscalar.cli --input inputs/example.png --output-dir outputs/custom --engine pillow_lanczos --scale 2
python -m upscalar.cli --list-engines
python -m upscalar.cli --history
```
...
```bash
export UPSCALAR_REALESRGAN_BIN=/path/to/realesrgan-ncnn-vulkan
```
...
このプロジェクト配下に導入した場合は `external/realesrgan-ncnn-vulkan/` を自動検出します。モデルディレクトリを変えたい場合は `UPSCALAR_REALESRGAN_MODEL_DIR` を指定してください。
...
```bash
export UPSCALAR_SPANDREL_MODEL_DIR=/path/to/weights
export UPSCALAR_SPANDREL_MODEL=/path/to/HAT_SRx4.pth
```
...
A-ESRGAN は4xモデルが中心です。GUIでは `multi`, `single`, `custom` を選べます。`custom` は後方互換用に `UPSCALAR_AESRGAN_MODEL` を見ます。
